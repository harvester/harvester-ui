...
export default {
  ...
  data() {
    ...
    if (roleContext === 'CLUSTER') {
      this.value.metadata.labels[HARVESTER_ROLE_CONTEXT] = 'cluster';
    } else if (roleContext === 'NAMESPACE') {
      this.value.metadata.labels[HARVESTER_ROLE_CONTEXT] = 'namespace';
    }
  ...
  methods: {
    ...
    createInheritedRules(parent, res = [], showParent = true) {
      ...
      this.$router.push({ path: '/c/local/auth/roles' });
    },
  }
};
</script>
